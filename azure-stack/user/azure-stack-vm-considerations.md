...
- VM ディスクの場合、Azure Stack のディスク IOPS (Input/Output Operations Per Second) は、ディスクの種類ではなく仮想マシン (VM) サイズの関数です。 つまり、Standard_Fs シリーズの VM では、ディスクの種類として SSD と HDD のどちらを選択した場合でも、1 つの追加データ ディスクの IOPS 制限は 2,300 IOPS です。
