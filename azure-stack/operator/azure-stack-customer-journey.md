...
1. 目的のユース ケースを説明し、要件を示します。
2. 使用する課金モデルを決定し、契約を確認して承認します。
3. [デプロイ ワークシート](azure-stack-deployment-worksheet.md)に記入して、すべてのデプロイの前提条件が満たされデプロイの準備ができていることを確認します。
4. 必要なすべての電源や冷却装置、境界接続、およびその他の必要なデータセンター統合要件が整っていることを確認してデータセンターを準備します。
5. サブスクリプションの資格情報を提供し、提供されたデータに関する質問がある場合にサポートできるように、デプロイ時には対応できるようにしておきます。

# <a name="partnertabpartner"></a>[パートナー](#tab/partner)

1. 計画フェーズ:
   - お客様の要件に基づいてソリューション オプションをお勧めします。
   - 必要な場合は、概念実証 (POC) を提案します。
   - 取引関係を確立します。
   - サポートのレベルを決定します。
2. 発注プロセス:
   - お客様と交わす必要なコントラクトを準備します。
   - お客様の注文書を作成します。
   - 納入のタイムラインを決定します。
   - 必要な場合は、お客様がマイクロソフトと連絡を取れるようにします。
3. デプロイ前準備
   - お客様に必要なトレーニングを提供して、デプロイのすべての前提条件とデータセンター統合オプションが確実に理解できるようにします。
   - 完全性と正確性を確実にするために、お客様が収集したデータを検証することを支援します。
4. 工場出荷時プロセス
   - 最新の検証されたベースライン ビルドを適用します。
   - 必要な Microsoft デプロイ ツールキットを適用します。
5. ハードウェア納入:
   - お客様のサイトにハードウェアを出荷します。
6. オンサイト デプロイ
   - デプロイはオンサイト エンジニアが実施します。
   - ラックとスタック。
   - ハードウェア ライフサイクル ホスト (HLH) デプロイ。
   - Azure Stack のデプロイ。
   - お客様にハンドオフします。

# <a name="microsofttabmicro"></a>[Microsoft](#tab/micro)

1. プリセールス サポートを提供するパートナーと連携します。
2. 必要に応じてソフトウェアのライセンスとコントラクトの準備をします。
3. データセンター統合の要件を収集するため必要なツールとお客様向けのドキュメントを備えます。
4. 月 1 回の周期で最新のベースライン ビルドとツール チェーンの更新プログラムを備えます。
5. Microsoft サポート エンジニアはデプロイの問題を支援します。
