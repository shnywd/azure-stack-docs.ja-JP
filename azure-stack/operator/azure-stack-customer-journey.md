...
|**お客様**|目的のユース ケースを説明し、要件を示します。|使用する課金モデルを決定し、契約を確認して承認します。|[デプロイ ワークシート](azure-stack-deployment-worksheet.md)に記入して、すべてのデプロイの前提条件が満たされデプロイの準備ができていることを確認します。|該当なし|必要なすべての電源や冷却装置、境界接続、およびその他の必要なデータセンター統合要件が整っていることを確認してデータセンターを準備します。|サブスクリプションの資格情報を提供し、提供されたデータに関する質問がある場合にサポートできるように、デプロイ時には対応できるようにしておきます。|
| | | | | | | |
